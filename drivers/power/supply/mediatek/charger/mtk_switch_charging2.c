...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (!mtk_pe20_get_is_connect(info) && !mtk_pe_get_is_connect(info) &&
		...
		pdata->input_current_limit_by_aicl = -1; //Bug506950,zhaosidong.wt,ADD,20191121,reset aicl
	}
...
//Bug 523166,zhaosidong.wt,ADD,20191226,no need to limit current by thermal in LPM
