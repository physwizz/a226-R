	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (!cti->ignore_usb) {
		...
		case CHARGING_HOST:
			...
			break;
		case NONSTANDARD_CHARGER:
			cable_type_notifier_set_attached_dev(CABLE_TYPE_NONSTANDARD);
#ifdef CONFIG_EXTCON_USB_CHG
			info->vbus_state = 1;
#endif
			break;
		case STANDARD_CHARGER:		/* AC : ~1A */
		case APPLE_2_1A_CHARGER:	/* 2.1A apple charger */
		case APPLE_1_0A_CHARGER:	/* 1A apple charger */
		case APPLE_0_5A_CHARGER:	/* 0.5A apple charger */
			cable_type_notifier_set_attached_dev(CABLE_TYPE_TA);
#ifdef CONFIG_EXTCON_USB_CHG
			info->vbus_state = 0;
#endif
			break;
