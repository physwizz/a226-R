...
void batt_cisd_init(struct mtk_battery *chip)
{
...
ssize_t cisd_store_attrs(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	const ptrdiff_t offset = attr - cisd_attrs;
	int ret = -EINVAL;
...
int batt_cisd_create_attrs(struct device *dev)
{
	unsigned long i = 0;
	int rc = 0;
