...
static char* battery_name[] = {"W1_SCUD_4V4_5000mAh","W1_BYD_4V4_5000mAh"};
...
void fgauge_get_profile_id(void)
{
	...
	battery_node = of_find_node_by_name(NULL, "battery");
	if (!battery_node) {
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		//zhaosidong.wt, 2020/12/16, battery id detection
		if (abs(id_volt - g_battery_id_voltage[id]) < 100) {
			gm.battery_id = id;
			break;
