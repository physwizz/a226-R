...
static int mtk_nanohub_get_data(struct hf_device *hfdev,
		int sensor_type, int32_t *data)
{
	int err = 0;
	struct data_unit_t data_t;

	pr_err("mtk_nanohub_get_data\n");

	err = mtk_nanohub_get_data_from_hub(type_to_id(sensor_type), &data_t);
	if (err < 0)
		return -1;

	switch (sensor_type) {
	case SENSOR_TYPE_PROXIMITY:
		data[0] = data_t.proximity_t.steps;
		pr_err("mtk_nanohub_get_data proximity raw data = %d\n", data[0]);
		break;
	default:
		pr_err("Unknown sensor_type %d\n", sensor_type);
		return -EINVAL;
	}
...
static int mtk_nanohub_create_manager(void)
{
	...
	hf_dev->get_data = mtk_nanohub_get_data;
