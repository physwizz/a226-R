...
static int hf_manager_get_data(struct hf_client *client,
		uint8_t sensor_type, int32_t *data)
{
	struct hf_manager *manager = NULL;
	struct hf_device *device = NULL;
	...
	if (device->get_data)
		ret = device->get_data(device, sensor_type, data);
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case HF_MANAGER_REQUEST_CUST_DATA:
		...
		if (copy_to_user(ubuf, &packet, sizeof(packet)))
			return -EFAULT;
		break;
	case HF_MANAGER_REQUEST_GET_DATA:
		pr_err("HF_MANAGER_REQUEST_GET_DATA\n");

		if (!test_bit(sensor_type, sensor_list_bitmap))
			return -EINVAL;

		pr_err("sensor_type = %d\n", sensor_type);

		if (hf_manager_get_data(client, sensor_type, data))
			return -EINVAL;

		memcpy(packet.byte, data, sizeof(data));
		if (copy_to_user(ubuf, &packet, sizeof(packet)))
			return -EFAULT;
		break;
	default:
