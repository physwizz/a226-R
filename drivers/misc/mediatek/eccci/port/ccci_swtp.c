...
/*Bug613556 chenduoyin.wt  20210223 Add swtp feature start*/
...
/*Bug604661 chenduoyin.wt  20210713 mtk patch optimization swtp feature start*/
...
/*Bug604661 chenduoyin.wt  20210713 mtk patch optimization swtp feature end*/
/*Bug613556 chenduoyin.wt  20210223 Add swtp feature end*/
...
/*Bug613556 chenduoyin.wt  20210223 Add swtp feature*/
...
/*Bug613556 chenduoyin.wt  20210223 Add swtp feature*/
       ...
       if ((swtp->gpio_state[0] == SWTP_EINT_PIN_PLUG_IN)&&(swtp->gpio_state[1] == SWTP_EINT_PIN_PLUG_OUT)&&(swtp->gpio_state[2] == SWTP_EINT_PIN_PLUG_IN)&&(swtp->gpio_state[3] == SWTP_EINT_PIN_PLUG_OUT)&&(swtp->gpio_state[4] == SWTP_EINT_PIN_PLUG_OUT))
       {
               ...
               CCCI_LEGACY_ERR_LOG(swtp->md_id, SYS,
			"wttest17-swtp-%s>>tx_power_mode =SWTP_NO_TX_POWER= %d\n", __func__,swtp->tx_power_mode);
        }
        else
        {
               swtp->tx_power_mode = SWTP_NO_TX_POWER;
                CCCI_LEGACY_ERR_LOG(swtp->md_id, SYS,
			"wttest18-swtp-%s>>tx_power_mode =SWTP_DO_TX_POWER= %d\n", __func__,swtp->tx_power_mode);
        }
       /*
	for (i = 0; i < MAX_PIN_NUM; i++) {
        CCCI_LEGACY_ERR_LOG(swtp->md_id, SYS,
...
static void swtp_init_delayed_work(struct work_struct *work)
 {
	...
	CCCI_NORMAL_LOG(-1, SYS, "swtp_init delayed work start\n");
	CCCI_BOOTUP_LOG(-1, SYS, "swtp_init delayed work start\n");
...
SWTP_INIT_END:
	CCCI_BOOTUP_LOG(md_id, SYS, "swtp_init delayed work end: ret = %d\n", ret);
	CCCI_NORMAL_LOG(md_id, SYS, "swtp_init delayed work end: ret = %d\n", ret);
...
/*Bug604661 chenduoyin.wt  20210713 mtk patch optimization swtp feature end*/
	...
	CCCI_BOOTUP_LOG(md_id, SYS, "swtp_init work scheduled\n");
	return 0;
}
