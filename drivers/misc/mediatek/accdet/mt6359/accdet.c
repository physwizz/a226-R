...
/* for headset pole voltage range judge(mv)  */
#define MIN_VOL_3_POLE		(0)
#define MAX_VOL_3_POLE		(580)
#define MIN_VOL_4_POLE		(581)
#define MAX_VOL_4_POLE		(1770)
#define PLUG_OUT_MIN_VOL	(0)
#define PLUG_OUT_MAX_VOL	(30)
...
static unsigned int check_pole_type(void)
{
	...
	if ((vol < MAX_VOL_4_POLE) && (vol > MIN_VOL_4_POLE)) {
		pr_info("[accdet] pole check:%d mv, AB=%d\n", vol,TYPE_AB_01);
		return TYPE_AB_01;
		/* } else if ((vol <= PLUG_OUT_MAX_VOL) && (vol>= PLUG_OUT_MIN_VOL)) {
		   ACCDET_DEBUG("[accdet] pole check:%d mv,AB=%d\n", vol, TYPE_AB_11);
		   return TYPE_AB_11; */
    } else if ((vol <MAX_VOL_3_POLE) && (vol >MIN_VOL_3_POLE)) {
	    pr_info("[accdet]pole check:%d mv,AB=%d\n", vol,TYPE_AB_00);
	    return TYPE_AB_00;
    }
    /*illegal state*/
