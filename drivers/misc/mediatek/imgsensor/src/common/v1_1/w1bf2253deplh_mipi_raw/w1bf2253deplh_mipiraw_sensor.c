...
#include "w1bf2253deplh_mipiraw_sensor.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "w1_bf2253_dep_lh_camera_sensor"
#define LOG_1 LOG_INF("W1_BF2253_DEP_LH, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
kal_bool W1_BF2253_DEP_LHDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = W1BF2253DEPLH_SENSOR_ID,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xe6,0x02);//20
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if ((*sensor_id == imgsensor_info.sensor_id) && (W1HI4821QREARQT_SENSOR_ID == main_sensor_id)) {
				LOG_INF("2nd dualcam i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
	...
	if ((*sensor_id != imgsensor_info.sensor_id) || (W1HI4821QREARQT_SENSOR_ID != main_sensor_id)) {
		...
		*sensor_id = 0xFFFFFFFF;
		pr_err("2nd dualcam sensor id error,main sensor id:%x,depth sensor id:%x\n",main_sensor_id,*sensor_id);
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	W1_BF2253_DEP_LHDuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
