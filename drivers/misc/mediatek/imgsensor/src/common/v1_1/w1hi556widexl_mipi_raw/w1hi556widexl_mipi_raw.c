...
#include "w1hi556widexl_mipi_raw.h"
...
void dumpEEPROMData1(int u4Length,u8* pu1Params)
{
	int i = 0;
...
#define MODULE_GROUP_FLAG 0x400
#define MODULE_GROUP1_ADDR 0x0401
#define MODULE_GROUP2_ADDR 0x0409
#define MODULE_GROUP3_ADDR 0x0411

#define AWB_GROUP_FLAG 0x0419
#define AWB_GROUP1_FLAG 0x041A
#define AWB_GROUP2_FLAG 0x042B
#define AWB_GROUP3_FLAG 0x043C

#define LSC_GROUP_FLAG 0x044D
#define LSC_GROUP1_FLAG 0x044E
#define LSC_GROUP2_FLAG 0x0B9B
#define LSC_GROUP3_FLAG 0x12E8
//-bug604664, liuxingyu, add, 2020/12/18, add for hi556 eeprom bring up
...
static int read_hi556_module_info(void)
{
	...
	/* read flag */
	write_cmos_sensor_8(OTP_SET_ADDR_H, (MODULE_GROUP_FLAG>>8)&0xff);
	write_cmos_sensor_8(OTP_SET_ADDR_L, (MODULE_GROUP_FLAG)&0xff);
	...
	if(otp_grp_flag == 0x01)
		...
		LOG_INF("no module info OTP hi556_data_info\n");
		return 0;
	}
...
#ifdef HI556_OTP_DUMP
	dumpEEPROMData1(MODULE_INFO_SIZE,&hi556_data_info[0]);
#endif
...
static int read_hi556_awb_info(void)
{
	...
	/* read flag */
	write_cmos_sensor_8(OTP_SET_ADDR_H, (AWB_GROUP_FLAG>>8)&0xff);
	write_cmos_sensor_8(OTP_SET_ADDR_L, (AWB_GROUP_FLAG)&0xff);
	write_cmos_sensor_8(OTP_RW_FLAG, 0x01);
	otp_grp_flag = read_cmos_sensor(OTP_READ_FLAG);
	LOG_INF("awb otp_grp_flag = 0x%x",otp_grp_flag);
	...
	if(otp_grp_flag == 0x01)
		...
		LOG_INF("no awb OTP hi556_data_info\n");
		return 0;
	}
...
#ifdef HI556_OTP_DUMP
	dumpEEPROMData1(AWB_DATA_SIZE,&hi556_data_awb[0]);
#endif
...
static int read_hi556_lsc_info(void)
{
	...
	/* read flag */
	write_cmos_sensor_8(OTP_SET_ADDR_H, (LSC_GROUP_FLAG>>8)&0xff);
	write_cmos_sensor_8(OTP_SET_ADDR_L, (LSC_GROUP_FLAG)&0xff);
	write_cmos_sensor_8(OTP_RW_FLAG, 0x01);
	otp_grp_flag = read_cmos_sensor(OTP_READ_FLAG);
	LOG_INF("lsc otp_grp_flag = 0x%x",otp_grp_flag);

	/* select group */
	if(otp_grp_flag == 0x01)
		lsc_start_addr = LSC_GROUP1_FLAG;
	else if(otp_grp_flag == 0x13)
		lsc_start_addr = LSC_GROUP2_FLAG;
	else if(otp_grp_flag == 0x37)
		lsc_start_addr = LSC_GROUP3_FLAG;
	else {
		LOG_INF("no awb OTP hi556_data_info\n");
		return 0;
	}
...
#ifdef HI556_OTP_DUMP
	dumpEEPROMData1(LSC_DATA_SIZE,&hi556_data_lsc[0]);
#endif
