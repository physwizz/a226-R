 ...
 * Filename:
 * ---------
 *     W1OV48B2QREARXL2Qmipi_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
 ...
 * Setting version:
 * ------------
 *   update full pd setting for W1OV48B2QREARXL2QEB_03B
 *------------------------------------------------------------------------------
 * Upper this line, this part is controlled by CC/CQ. DO NOT MODIFY!!
 *============================================================================
 ****************************************************************************/

#define PFX "W1OV48B2QREARXL_camera_sensor"
...
#include "w1ov48b2qrearxl_mipi_raw.h"
#include "w1ov48b2qrearxl_Sensor_setting.h"
...
#ifdef VENDOR_EDIT
#define DEVICE_VERSION_W1OV48B2QREARXL     "w1ov48b2qrearxl"
extern void register_imgsensor_deviceinfo(char *name, char *version, u8 module_id);
static kal_uint8 deviceInfo_register_value = 0x00;
//static kal_uint32 streaming_control(kal_bool enable);
#endif

//+bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
#define W1OV48B2QREARXL_PDC_ENABLE 1
#define W1OV48B2QREARXL_PDC_OTP_DUMP 0
#define W1OV48B2QREARXL_CROSSSTALK_ENABLE 1
#define W1OV48B2QREARXL_CROSSSTALK_OTP_DUMP 0
//+bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.

//+bug604664 liudijn, add, 2020/12/25,3ts w1ov48b2qrearxl eeprom bring up.
#define W1OV48B2QREARXL_OTP_ENABLE 1
#if W1OV48B2QREARXL_OTP_ENABLE
#define EEPROM_BL24SA64D_ID 0xA0
static kal_uint16 w1ov48b2qrearxl_read_eeprom(kal_uint32 addr)
{
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = W1OV48B2QREARXL_SENSOR_ID,
	...
	//+bug604664 liudijin, add, 2021/01/19,3st ov48b2q sensor add 1280*720 hs video support.
	.hs_video = {
		.pclk = 115200000,
		.linelength = 576,
		.framelength = 1666,
...
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[5] = {
    /* Preview,capture mode setting 496(pxiel)*1496*/
    {
...
static kal_uint16 w1ov48b2qrearxl_table_write_cmos_sensor(
...
static kal_uint16 w1ov48b2qrearxl_burst_write_cmos_sensor(
					kal_uint16 *para, kal_uint32 len)
{
	char puSendCmd[I2C_BUFFER_LEN];
	kal_uint32 tosend, IDX;
...
static void sensor_init(void)
{
	...
	w1ov48b2qrearxl_table_write_cmos_sensor(
		addr_data_pair_init_w1ov48b2qrearxl,
		sizeof(addr_data_pair_init_w1ov48b2qrearxl) / sizeof(kal_uint16));
...
static void preview_setting(void)
{
	...
	pr_debug("preview_setting RES_4000x3000_30fps\n");	
	_length = sizeof(addr_data_pair_preview_w1ov48b2qrearxl) / sizeof(kal_uint16);
	if(!_is_seamless) {
	w1ov48b2qrearxl_table_write_cmos_sensor(
		addr_data_pair_preview_w1ov48b2qrearxl,
		_length);
	} else {
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_preview_w1ov48b2qrearxl,
			sizeof(addr_data_pair_preview_w1ov48b2qrearxl));
		_size_to_write += _length;
	}
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if(!_is_seamless) {
	w1ov48b2qrearxl_table_write_cmos_sensor(
		addr_data_pair_capture_w1ov48b2qrearxl,
		_length);
	} else {
		pr_debug("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_capture_w1ov48b2qrearxl,
			sizeof(addr_data_pair_capture_w1ov48b2qrearxl));
		_size_to_write += _length;
	}
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	pr_debug("normal_video_setting RES_4000x3000_zsl_30fps\n");
	_length = sizeof(addr_data_pair_video_w1ov48b2qrearxl) / sizeof(kal_uint16);
	if(!_is_seamless) {
	w1ov48b2qrearxl_table_write_cmos_sensor(
		addr_data_pair_video_w1ov48b2qrearxl,
		_length);
	} else {
		pr_debug("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_video_w1ov48b2qrearxl,
			sizeof(addr_data_pair_video_w1ov48b2qrearxl));
		_size_to_write += _length;
	}
...
static void hs_video_setting(void)
{
	...
	_length = sizeof(addr_data_pair_hs_video_w1ov48b2qrearxl) / sizeof(kal_uint16);
	if(!_is_seamless) {
		w1ov48b2qrearxl_table_write_cmos_sensor(
		addr_data_pair_hs_video_w1ov48b2qrearxl,
		_length);
	} else {
		pr_debug("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_hs_video_w1ov48b2qrearxl,
			sizeof(addr_data_pair_hs_video_w1ov48b2qrearxl));
		_size_to_write += _length;
	}
...
static void slim_video_setting(void)
{
	...
	_length = sizeof(addr_data_pair_slim_video_w1ov48b2qrearxl) / sizeof(kal_uint16);
	if(!_is_seamless) {
		w1ov48b2qrearxl_table_write_cmos_sensor(
		addr_data_pair_slim_video_w1ov48b2qrearxl,
		_length);
	} else {
		pr_debug("%s _is_seamless %d, _size_to_write %d\n",
			__func__, _is_seamless, _size_to_write);
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_slim_video_w1ov48b2qrearxl,
			sizeof(addr_data_pair_slim_video_w1ov48b2qrearxl));
		_size_to_write += _length;
	}
...
static void custom3_setting(void)
{
	...
	LOG_INF("E\n");
	w1ov48b2qrearxl_burst_write_cmos_sensor(addr_data_pair_custom3,
...
#ifdef VENDOR_EDIT
/*add for ModuleSN*/
static kal_uint8 gW1OV48B2QREARXL_SN[CAMERA_MODULE_SN_LENGTH];
static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gW1OV48B2QREARXL_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		...
		LOG_INF("W1OV48B2QREARXL_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gW1OV48B2QREARXL_SN[idx]);
	}
}

#define  CAMERA_MODULE_INFO_LENGTH  (8)
static kal_uint8 gW1OV48B2QREARXL_CamInfo[CAMERA_MODULE_INFO_LENGTH];
/*add for google ARCode Feature */
static void read_eeprom_CamInfo(void)
{
	...
	for (idx = 0; idx <12; idx++) {
		...
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA0);
		LOG_INF("W1OV48B2QREARXL_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	gW1OV48B2QREARXL_CamInfo[0] = get_byte[0];
	gW1OV48B2QREARXL_CamInfo[1] = get_byte[1];
	gW1OV48B2QREARXL_CamInfo[2] = get_byte[6];
	gW1OV48B2QREARXL_CamInfo[3] = get_byte[7];
	gW1OV48B2QREARXL_CamInfo[4] = get_byte[8];
	gW1OV48B2QREARXL_CamInfo[5] = get_byte[9];
	gW1OV48B2QREARXL_CamInfo[6] = get_byte[10];
	gW1OV48B2QREARXL_CamInfo[7] = get_byte[11];
}

/*add for sensor Module SET*/
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == W1OV48B2QREARXL_SENSOR_ID)
				&& ((data_base == W1OV48B2QREARXL_STEREO_START_ADDR_WIDE)|| (data_base == W1OV48B2QREARXL_STEREO_START_ADDR_TELE))
			...
			write_eeprom_protect(1);
			msleep(6);
			if (data_base == W1OV48B2QREARXL_STEREO_START_ADDR_TELE) {
				pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE));
				msleep(6);
				pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+39));
				msleep(6);
				pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+40));
				msleep(6);
				pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+1556));
				msleep(6);
				pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+1557));
				msleep(6);
				pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+1558));
				msleep(6);
				pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+1559));
				msleep(6);
				pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_TELE+1560));
				msleep(6);
			} else {
				pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE));
				msleep(6);
				pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+39));
				msleep(6);
				pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+40));
				msleep(6);
				pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+1556));
				msleep(6);
				pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+1557));
				msleep(6);
				pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+1558));
				msleep(6);
				pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+1559));
				msleep(6);
				pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(W1OV48B2QREARXL_STEREO_START_ADDR_WIDE+1560));
				msleep(6);
			}
			pr_debug("ov48b write_Module_data Write end\n");
		}else if ((pStereodata->uSensorId == W1OV48B2QREARXL_SENSOR_ID)
			&& (data_base == W1OV48B2QREARXL_AESYNC_START_ADDR)
			...
			pr_debug("readback main aesync: %x %x %x %x %x %x %x %x\n"
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+1)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+2)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+3)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+4)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+5)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+6)
				, read_cmos_eeprom_8(W1OV48B2QREARXL_AESYNC_START_ADDR+7));
		}else {
...
{
	w1ov48b2qrearxl_table_write_cmos_sensor(ov48b_PDC_setting,
		sizeof(ov48b_PDC_setting)/sizeof(kal_uint16));
	w1ov48b2qrearxl_burst_write_cmos_sensor(ov48b_PDC_setting_burst,
...
#ifdef W1OV48B2QREARXL_PDC_ENABLE
static kal_uint16 w1ov48b2qrearxl_PDC_setting[8*2];
static kal_uint16 w1ov48b2qrearxl_PDC_setting_burst[720*2];

static void w1ov48b2qrearxl_read_sensor_pdc_Cali(void)
{
	kal_uint16 idx = 0, eeprom_PDC_addr = 0x1143, sensor_PDC_addr1 = 0x5C0E, sensor_PDC_addr2 = 0x5900;
	#if W1OV48B2QREARXL_PDC_OTP_DUMP
        kal_uint16 idx2 = 0;
        kal_uint16 idx3 = 0;
	#endif

	for (idx = 0; idx <8; idx++) {
		eeprom_PDC_addr = 0x1143 + idx;
		sensor_PDC_addr1 = 0x5C0E + idx;
		w1ov48b2qrearxl_PDC_setting[2*idx] = sensor_PDC_addr1;
		w1ov48b2qrearxl_PDC_setting[2*idx + 1] = read_cmos_eeprom_8(eeprom_PDC_addr);
                #if W1OV48B2QREARXL_PDC_OTP_DUMP
                idx2 = 2*idx + 1;
                LOG_INF( "setting sensor_PDC_addr1[%d]=0x%x, PDC_addr2_data[%d]=0x%x \n",
                    idx, sensor_PDC_addr1,idx2, w1ov48b2qrearxl_PDC_setting[2*idx + 1]);
                #endif
	}
	for (idx = 8; idx < 728; idx++) {
		eeprom_PDC_addr = 0x1143 + idx;
		sensor_PDC_addr2 = 0x5900 + idx - 8;
		w1ov48b2qrearxl_PDC_setting_burst[2*(idx-8)] = sensor_PDC_addr2;
		w1ov48b2qrearxl_PDC_setting_burst[2*(idx-8) + 1] = read_cmos_eeprom_8(eeprom_PDC_addr);
                #if W1OV48B2QREARXL_PDC_OTP_DUMP
                idx3 = 2*(idx-8) + 1;
                LOG_INF( "dump: sensor_PDC_addr2[%d]=0x%x, PDC_addr2_data[%d]=0x%x \n",
                    idx, sensor_PDC_addr2,idx3, w1ov48b2qrearxl_PDC_setting_burst[2*(idx-8) + 1]);
                #endif

	}
}

static void w1ov48b2qrearxl_write_sensor_PDC(void)
{
	#if W1OV48B2QREARXL_PDC_OTP_DUMP
	kal_uint16  value5000 = 0;
	kal_uint16 value5900 = 0;
	kal_uint16 value5901 = 0;
	kal_uint16 value5902 = 0;
	#endif
	//write_cmos_sensor(0x5000,0xcf); //enable sensor pdc
	w1ov48b2qrearxl_table_write_cmos_sensor(w1ov48b2qrearxl_PDC_setting,
		sizeof(w1ov48b2qrearxl_PDC_setting)/sizeof(kal_uint16));
	w1ov48b2qrearxl_burst_write_cmos_sensor(w1ov48b2qrearxl_PDC_setting_burst,
		sizeof(w1ov48b2qrearxl_PDC_setting_burst)/sizeof(kal_uint16));
	#if W1OV48B2QREARXL_PDC_OTP_DUMP
	value5000 = read_cmos_sensor(0x5000);
	value5900 = read_cmos_sensor(0x5900);
	value5901 = read_cmos_sensor(0x5901);
	value5902 = read_cmos_sensor(0x5902);
	LOG_INF( "w1ov48b2qrearxl_write_sensor_PDC end  0x5000_value:0x%x 0x5900_value:0x%x  0x5901_value:0x%x  0x5902_value:0x%x",
		value5000,value5900,value5901,value5902);
	#endif
}
#endif

#ifdef W1OV48B2QREARXL_CROSSSTALK_ENABLE

static kal_uint16 w1ov48b2qrearxl_crosstalk_setting[288*2];

static void w1ov48b2qrearxl_read_sensor_crosstalk_Cali(void)
{
	kal_uint16 idx = 0, eeprom_crosstalk_addr = 0x141D, sensor_crosstalk_addr = 0x53C0;
        #if W1OV48B2QREARXL_CROSSSTALK_OTP_DUMP
        kal_uint16 idx2 = 0;
        #endif
	for (idx = 0; idx < 288; idx++) {
		eeprom_crosstalk_addr = 0x141D + idx;
		sensor_crosstalk_addr = 0x53C0 + idx;
		w1ov48b2qrearxl_crosstalk_setting[2*idx] = sensor_crosstalk_addr;
		w1ov48b2qrearxl_crosstalk_setting[2*idx + 1] = read_cmos_eeprom_8(eeprom_crosstalk_addr);
                #if W1OV48B2QREARXL_CROSSSTALK_OTP_DUMP
                idx2 = 2*idx + 1;
                LOG_INF( "dump: sensor_crosstalk_addr[%d]=0x%x, sensor_crosstalk_data[%d]=0x%x \n",
                    idx, sensor_crosstalk_addr,idx2, w1ov48b2qrearxl_crosstalk_setting[2*idx + 1]);
                #endif
	}
}

static void w1ov48b2qrearxl_write_sensor_crosstalk(void)
{
	#if W1OV48B2QREARXL_CROSSSTALK_OTP_DUMP
	kal_uint16  crosstalk_5000_value = 0;
	kal_uint16  crosstalk_5380_value = 0;
	kal_uint16  crosstalk_53C0_value = 0;
	kal_uint16  crosstalk_53C1_value = 0;
	kal_uint16  crosstalk_53C2_value = 0;
	#endif
	//write_cmos_sensor(0x5000,0xCB); //enable sensor crosstalk
	//write_cmos_sensor(0x5380,0x0F); //enable sensor crosstalk
	w1ov48b2qrearxl_table_write_cmos_sensor(w1ov48b2qrearxl_crosstalk_setting,
		sizeof(w1ov48b2qrearxl_crosstalk_setting)/sizeof(kal_uint16));
	#if W1OV48B2QREARXL_CROSSSTALK_OTP_DUMP
	crosstalk_5000_value = read_cmos_sensor(0x5000);
	crosstalk_5380_value = read_cmos_sensor(0x5380);
	crosstalk_53C0_value = read_cmos_sensor(0x53C0);
	crosstalk_53C1_value = read_cmos_sensor(0x53C1);
	crosstalk_53C2_value = read_cmos_sensor(0x53C2);
	LOG_INF( "w1ov48b2qrearxl_write_sensor_crosstalk end  cross5000:0x%x  crosstalk_5380_values:0x%x cross53C0:0x%x  cross53C1:0x%x  cross53C2:0x%x",
	crosstalk_5000_value, crosstalk_5380_value, crosstalk_53C0_value,crosstalk_53C1_value,crosstalk_53C2_value);
	#endif
...
static struct stCAM_CAL_DATAINFO_STRUCT st_rear_w1ov48b2qrearxl_eeprom_data ={
        .sensorID= W1OV48B2QREARXL_SENSOR_ID,
        .deviceID = 0x01,
        .dataLength = 0x153F,
        .sensorVendorid = 0x06000000,
        .vendorByte = {1,2,3,4},
        .dataBuffer = NULL,
};

static struct stCAM_CAL_CHECKSUM_STRUCT st_rear_w1ov48b2qrearxl_Checksum[11] =
{
    ...
    {PDAF_PROC3_ITEM,0x0D50,0x0D50,0x113C,0x113D,0x55},
    {OV48B2Q_PDC_ITEM,0x113E,0x113E,0x141A,0x141B,0x55},
    {OV48B2Q_CT_ITEM,0x141C,0x141C,0x153C,0x153D,0x55},
    {TOTAL_ITEM,0x0000,0x0000,0x153D,0x153E,0x55},
	...
	if (*sensor_id == imgsensor_info.sensor_id) {
		LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
			imgsensor.i2c_write_id, *sensor_id);
                        //+bug604664 liudijn, add, 2020/12/25,3ts w1ov48b2qrearxl eeprom bring up.
                        #if W1OV48B2QREARXL_OTP_ENABLE
                        size = imgSensorReadEepromData(&st_rear_w1ov48b2qrearxl_eeprom_data,st_rear_w1ov48b2qrearxl_Checksum);
                        if(size != st_rear_w1ov48b2qrearxl_eeprom_data.dataLength || (st_rear_w1ov48b2qrearxl_eeprom_data.sensorVendorid >> 24)!= w1ov48b2qrearxl_read_eeprom(0x0001)) {
                            pr_err("get eeprom data failed\n");
                            if(st_rear_w1ov48b2qrearxl_eeprom_data.dataBuffer != NULL) {
                                kfree(st_rear_w1ov48b2qrearxl_eeprom_data.dataBuffer);
                                st_rear_w1ov48b2qrearxl_eeprom_data.dataBuffer = NULL;
                            }
                            //*sensor_id = 0xFFFFFFFF;
                            //return ERROR_SENSOR_CONNECT_FAIL;
                        } else {
                            pr_info("get eeprom data success size:%d\n", size);
                            imgSensorSetEepromData(&st_rear_w1ov48b2qrearxl_eeprom_data);
                        }
                        #endif
                        //-bug604664 liudijn, add, 2020/12/25,3ts w1ov48b2qrearxl eeprom bring up.
                        ...
                        //+bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
                        #ifdef W1OV48B2QREARXL_PDC_ENABLE
                        w1ov48b2qrearxl_read_sensor_pdc_Cali();
                        w1ov48b2qrearxl_read_sensor_crosstalk_Cali();
                        #endif
                        //-bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
			...
			if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_r0", DEVICE_VERSION_W1OV48B2QREARXL, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
	...
	//+bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
	#ifdef W1OV48B2QREARXL_PDC_ENABLE
	w1ov48b2qrearxl_write_sensor_PDC();
	LOG_INF("w1ov48b2qrearxl_write_sensor_PDC \n");
	#endif
	//-bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
	#ifdef VENDOR_EDIT
	/*Driver add for writing PDC data*/
	write_sensor_PDC();
	#endif
	spin_lock(&imgsensor_drv_lock);
					...
					MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	custom3_setting();
	//+bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
	#if W1OV48B2QREARXL_CROSSSTALK_ENABLE
	w1ov48b2qrearxl_write_sensor_crosstalk();
	#endif
	//-bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable sensor pdc and crosstalk funxtion.
	return ERROR_NONE;
}   /*  Custom3*/
					...
					MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	custom5_setting();
	//+bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable crosstalk for custom5.
	#if W1OV48B2QREARXL_CROSSSTALK_ENABLE
	w1ov48b2qrearxl_write_sensor_crosstalk();
	#endif
	//-bug604664 liudijin, add, 2021/01/26,3st ov48b2q sensor enable crosstalk for custom5.
...
//	int k = 0;
	...
	LOG_INF("seamless_switch %d, %d, %d, %d, %d sizeof(_i2c_data) %d\n",
		scenario_id, shutter, gain, shutter_2ndframe, gain_2ndframe, sizeof(_i2c_data));

	_length = sizeof(addr_data_pair_seamless_switch_step1_w1ov48b2qrearxl) / sizeof(kal_uint16);
	...
	memcpy((void *)(_i2c_data + _size_to_write),
		addr_data_pair_seamless_switch_step1_w1ov48b2qrearxl,
		sizeof(addr_data_pair_seamless_switch_step1_w1ov48b2qrearxl));
	_size_to_write += _length;
	...
	_length = sizeof(addr_data_pair_seamless_switch_step2_w1ov48b2qrearxl) / sizeof(kal_uint16);
	...
	memcpy((void *)(_i2c_data + _size_to_write),
		addr_data_pair_seamless_switch_step2_w1ov48b2qrearxl,
		sizeof(addr_data_pair_seamless_switch_step2_w1ov48b2qrearxl));
	_size_to_write += _length;
	...
	_length = sizeof(addr_data_pair_seamless_switch_step3_w1ov48b2qrearxl) / sizeof(kal_uint16);
	if (_size_to_write + _length > _I2C_BUF_SIZE) {
		pr_err("_too much i2c data for fast siwtch\n");
		return ERROR_NONE;
	}
	memcpy((void *)(_i2c_data + _size_to_write),
		addr_data_pair_seamless_switch_step3_w1ov48b2qrearxl,
		sizeof(addr_data_pair_seamless_switch_step3_w1ov48b2qrearxl));
	_size_to_write += _length;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
	//+bug604664 liudijin, add, 2021/01/21,3st ov48b2q sensor add seamless function support.
	case SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS:
		LOG_INF("seamless feature_id = %d\n", feature_id);
		...
		*pScenarios = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
		...
		case MSDK_SCENARIO_ID_CUSTOM5:
			...
			TimeOV = 1000/fpsOV;
			mdelay(TimeOV); //\u5982\u89e3\u4e0d\u6389\uff0c\u53ef\u4ee5delay 2*TimeOV
			//-bug604664,huangguoyong.wt, add, 2021/04/03, add hi4821q seamless code
			*pScenarios = MSDK_SCENARIO_ID_CAMERA_PREVIEW;
			break;
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
	...
	*(feature_data + 1) = imgsensor_info.min_shutter;
		...
		break;
#ifdef VENDOR_EDIT
		/*add for google ARCode Feature verify*/
		case SENSOR_FEATURE_GET_MODULE_INFO:
			LOG_INF("W1OV48B2QREARXL GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
			*(feature_data_32 + 1) = (gW1OV48B2QREARXL_CamInfo[1] << 24)
						| (gW1OV48B2QREARXL_CamInfo[0] << 16)
						| (gW1OV48B2QREARXL_CamInfo[3] << 8)
						| (gW1OV48B2QREARXL_CamInfo[2] & 0xFF);
			*(feature_data_32 + 2) = (gW1OV48B2QREARXL_CamInfo[5] << 24)
						| (gW1OV48B2QREARXL_CamInfo[4] << 16)
						| (gW1OV48B2QREARXL_CamInfo[7] << 8)
						| (gW1OV48B2QREARXL_CamInfo[6] & 0xFF);
			break;
		/* add for 18531 ModuleSN*/
		case SENSOR_FEATURE_GET_MODULE_SN:
			LOG_INF("W1OV48B2QREARXL GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
			if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
				*(feature_data_32 + 1) = (gW1OV48B2QREARXL_SN[4*(*feature_data_32) + 3] << 24)
							| (gW1OV48B2QREARXL_SN[4*(*feature_data_32) + 2] << 16)
							| (gW1OV48B2QREARXL_SN[4*(*feature_data_32) + 1] << 8)
							| (gW1OV48B2QREARXL_SN[4*(*feature_data_32)] & 0xFF);
			break;
		/*add for sensor Module SET*/
...
/* ITD: Modify Dualcam End */
	...
	case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
		LOG_INF("[dennis-debug]  SENSOR_FEATURE_GET_MIPI_PIXEL_RATE :%d", *feature_data);
			switch (*feature_data) {
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) =
...
/* ITD: Modify Dualcam End */
	...
	break;
	//+bug604664 liudijn, add, 2021/02/01,fixed 3ts w1ov48b2qrearxl remosaic takepicture over exposure issue.
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		case MSDK_SCENARIO_ID_CUSTOM4:
		default:
			*feature_return_para_32 = 2190; /*BINNING_AVERAGED*/
			break;
		}
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		...
		case MSDK_SCENARIO_ID_CUSTOM2:
			memcpy((void *)pvcinfo, (void *)&SENSOR_VC_INFO[3],
				...
				break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			//+bug604664 liudijn, add, 2021/01/15,fix 3ts w1ov48b2qrearxl dualcam pdaf crash.
