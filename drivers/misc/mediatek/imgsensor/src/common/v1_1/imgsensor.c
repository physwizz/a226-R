...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		if(psensor_inst->sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN){
			...
			else if(!strcmp(psensor_inst->psensor_list->name, "w1hi846frontst2_mipi_raw"))
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "SHENGTAI2");
			//+bug584789, qinduilin.wt, ADD, 2021/11/10, add front camera c8490 bringup
			else if(!strcmp(psensor_inst->psensor_list->name, "w1c8490fronttxd_mipi_raw"))
