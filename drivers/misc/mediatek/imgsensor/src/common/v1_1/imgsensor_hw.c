...
extern  void AFRegulatorCtrl_Main(int Stage);//EXTB P210730-04853 yaoguizhen.wt, add, 2021/8/4, for fix IIC communication  fail when apply af init setting
	...
	while (ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON) {
			if (ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
				...
				//+bug584789,zhanghengyuan.wt,ADD,2021/3/10,disable wide sensor while open dep sensor
				if(sensor_idx == 2 && ppwr_info->pin == IMGSENSOR_HW_PIN_RST){
					if (pdev->set != NULL)
		...
		enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	imgsensor_hw_power_sequence(
        ...
        //+EXTB P210730-04853 yaoguizhen.wt, add, 2021/8/4, for fix IIC communication  fail when apply af init setting
        if((strcmp(curr_sensor_name,SENSOR_DRVNAME_W1OV64B40REARTXD_MIPI_RAW)==0)
            ||(strcmp(curr_sensor_name,SENSOR_DRVNAME_W1HI4821QREARQT_MIPI_RAW)==0)
            ||(strcmp(curr_sensor_name,SENSOR_DRVNAME_W1HI4821QREARSY_MIPI_RAW)==0)
            ||(strcmp(curr_sensor_name,SENSOR_DRVNAME_W1OV48B2QREARXL_MIPI_RAW)==0)){
            if(pwr_status==IMGSENSOR_HW_POWER_STATUS_ON){
                AFRegulatorCtrl_Main(0);
                AFRegulatorCtrl_Main(1);
                mdelay(5);
                PK_INFO("AF_Open mdelay 5ms,curr_sensor_name is %s\n",curr_sensor_name);
            }else if(pwr_status==IMGSENSOR_HW_POWER_STATUS_OFF){
                AFRegulatorCtrl_Main(2);
                PK_INFO("AF_Close,curr_sensor_name is %s\n",curr_sensor_name);
            }
        }
        //-EXTB P210730-04853 yaoguizhen.wt, add, 2021/8/4, for fix IIC communication  fail when apply af init setting
