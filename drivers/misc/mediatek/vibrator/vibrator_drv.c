...
static ssize_t show_motor_type(struct device *dev, struct device_attribute *attr,
                 char *data)
{
	return sprintf(data, "%s\n", "COINDC");
}

static DEVICE_ATTR(motor_type, 0664, show_motor_type, NULL);
...
static int vib_mod_init(void)
{
	...
	ret = device_create_file(mtk_vibrator.dev, &dev_attr_motor_type);
	if (ret)
		VIB_DEBUG("device_create_file motor_type fail!\n");
