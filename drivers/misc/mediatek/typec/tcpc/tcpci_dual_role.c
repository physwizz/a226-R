				      ...
				      enum typec_role trole)
{
	...
	switch (trole) {
	case TYPEC_SOURCE:
		val = TCP_ROLE_PROP_PR_SRC;
		role = PD_ROLE_SOURCE;
		break;
	case TYPEC_SINK:
		val = TCP_ROLE_PROP_PR_SNK;
		role = PD_ROLE_SINK;
		break;
	default:
		return 0;
				      ...
				      enum typec_data_role data)
{
	...
	switch (data) {
	case TYPEC_HOST:
		val = TCP_ROLE_PROP_DR_HOST;
		role = PD_ROLE_DFP;
		break;
	case TYPEC_DEVICE:
		val = TCP_ROLE_PROP_DR_DEVICE;
		role = PD_ROLE_UFP;
		break;
	default:
		return 0;
					 ...
					 enum typec_role trole)
{
	...
	switch (trole) {
	case TYPEC_SINK:
		val = TCP_ROLE_PROP_VCONN_SUPPLY_NO;
		role = PD_ROLE_VCONN_OFF;
		break;
	case TYPEC_SOURCE:
		val = TCP_ROLE_PROP_VCONN_SUPPLY_YES;
...
{
	uint8_t role = TYPEC_ROLE_UNKNOWN;
	struct tcpc_device *tcpc = container_of(cap,
						struct tcpc_device, typec_caps);
//+Extb P210504-02856 ,lvyuanchuan.wt,modify,2021/06/14, Could't switch text is not display under This device;
	bool as_sink = true;
	...
	pr_info("%s as_sink : %d , type : %d\n",__func__,as_sink,type);
	switch (type) {
	case TYPEC_PORT_UFP:
		if(as_sink){
			return 0;
		}else{
			role = TYPEC_ROLE_TRY_SNK;
