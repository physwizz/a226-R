...
info_retry:
	...
	if (lcm_tp_name == TM_NT36672C_MODULE) {
		sprintf(Ctp_name, "TM,NT36672C,FW:0x%02x", ts->fw_ver);
		printk( "Nvt_tpfwver_show Ctp_name is : %s\n", Ctp_name);
	} else if (lcm_tp_name == DJN_NT36672C_MODULE) {
		sprintf(Ctp_name, "DJN,NT36672C,FW:0x%02x", ts->fw_ver);
		printk( "Nvt_tpfwver_show Ctp_name is : %s\n", Ctp_name);
	}
...
static int nvt_parse_dt(struct device *dev)
{
	...
	platdata->lcd_id = 0;
#endif
	input_info(true,dev,"%s: g_lcm_name=%d",__func__,lcm_tp_name);
	if(lcm_tp_name==TM_NT36672C_MODULE){
		of_property_read_string_index(np, "novatek,tm_fw_name", platdata->lcd_id, &platdata->firmware_name);
		...
		of_property_read_string_index(np, "novatek,tm_fw_name_mp", platdata->lcd_id, &platdata->firmware_name_mp);
		...
		of_property_read_string_index(np, "novatek,djn_fw_name", platdata->lcd_id, &platdata->firmware_name);
		if (platdata->firmware_name == NULL || strlen(platdata->firmware_name) == 0) {
			input_err(true, dev, "%s: Failed to get fw name\n", __func__);
			return -EINVAL;
		} else {
		...
		of_property_read_string_index(np, "novatek,djn_fw_name_mp", platdata->lcd_id, &platdata->firmware_name_mp);
		if (platdata->firmware_name_mp == NULL || strlen(platdata->firmware_name_mp) == 0) {
			input_err(true, dev, "%s: Failed to get mp fw name\n", __func__);
			return -EINVAL;
		} else {
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (strstr(saved_command_line,"nt36672c_fhdp_wt_dsi_vdo_cphy_90hz_tianma")){
		lcm_tp_name=TM_NT36672C_MODULE;
	...
	else if(strstr(saved_command_line,"nt36672c_fhdp_wt_dsi_vdo_cphy_90hz_djn")) {
		lcm_tp_name=DJN_NT36672C_MODULE;
		pr_info("[sec_input] %s : start\n", __func__);
