...
#define ILITEK_PROC_TOUCHSCREEN_FOLDER                   "touchscreen"
#define ILITEK_PROC_CTP_OPENSHORT_TEST_FILE            "ctp_openshort_test"

struct proc_dir_entry *ilitek_proc_touchscreen_dir;
struct proc_dir_entry *ilitek_proc_ctp_openshort_test_file = NULL;
...
static int __init proc_node_init(void)
{
    ...
    if (!strstr(saved_command_line,"ili7807s_fhdp_wt_dsi_vdo_cphy_90hz_txd"))
        return  -1;

    ilitek_proc_touchscreen_dir = proc_mkdir(ILITEK_PROC_TOUCHSCREEN_FOLDER , NULL);
    if (ilitek_proc_touchscreen_dir == NULL)  {
        printk(KERN_ERR "%s:ilitek_proc_touchpanel_dir file create failed!\n", __func__);
    }else{
#ifdef PROC_NODE
        ilitek_proc_ctp_openshort_test_file = proc_create(ILITEK_PROC_CTP_OPENSHORT_TEST_FILE,
		(S_IWUSR | S_IRUGO), ilitek_proc_touchscreen_dir, &proc_ctp_openshort_test_fops);
        if(ilitek_proc_ctp_openshort_test_file ==NULL){
            printk(KERN_ERR "%s: ilitek_proc_ctp_openshort_test_file create failed!\n", __func__);
            remove_proc_entry(ILITEK_PROC_TOUCHSCREEN_FOLDER, NULL);
        }else{
            printk("%s:  ilitek_proc_create PROC_CTP_OPENSHORT_TEST_FILE success",__func__);
        }
#endif
    }
    return  0;
