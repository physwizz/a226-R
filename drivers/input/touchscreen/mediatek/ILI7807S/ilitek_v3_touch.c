...
void ili_report_ap_mode(u8 *buf, int len)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		axis_info[ilits->finger].width_major = (((buf[(5 * i) + 1 + P5_X_DEMO_MODE_PACKET_LEN -1 ]) << 8) | (buf[(5 * i) + 2 + P5_X_DEMO_MODE_PACKET_LEN -1 ]));
		axis_info[ilits->finger].width_minor = (((buf[(5 * i) + 3 + P5_X_DEMO_MODE_PACKET_LEN -1 ]) << 8) | (buf[(5 * i) + 4 + P5_X_DEMO_MODE_PACKET_LEN -1 ]));
...
#if AXIS_PACKET
	for (i = 0; i < 16; i++){
		ILI_DBG("Other Information byte[%d]:%02x\n", i, buf[P5_X_DEMO_MODE_PACKET_LEN + P5_X_DEMO_MODE_AXIS_LEN - 1 + i]);
	}
#endif
...
#if AXIS_PACKET
	axis_addr = (2 * ilits->xch_num * ilits->ych_num) + (ilits->stx * 2) + (ilits->srx * 2) + 2 * 2 + 30;
	ilits->palmflag = ((buf[axis_addr + P5_X_DEMO_MODE_AXIS_LEN + 2] & 0x08) >> 3);
	ILI_DBG("palmflag = %d\n", ilits->palmflag);
#endif
...
#if AXIS_PACKET
		axis_info[ilits->finger].degree = buf[(5 * i) + (axis_addr)];
		axis_info[ilits->finger].width_major = (((buf[(5 * i) + 1 + (axis_addr)]) << 8) | (buf[(5 * i) + 2 + (axis_addr)]));
		axis_info[ilits->finger].width_minor = (((buf[(5 * i) + 3 + (axis_addr)]) << 8) | (buf[(5 * i) + 4 + (axis_addr)]));
		ILI_DBG("finger = %d, degree = %d, width_major = %d, width_minor = %d\n", ilits->finger, axis_info[ilits->finger].degree, axis_info[ilits->finger].width_major, axis_info[ilits->finger].width_minor);
#endif
...
#if AXIS_PACKET
	for (i = 0; i < 16; i++){
		ILI_DBG("Other Information byte[%d]:%02x\n", i, buf[axis_addr + P5_X_DEMO_MODE_AXIS_LEN + i]);
	}
#endif
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	u8 *ges = NULL;
	if (ilits->position_high_resolution == OFF) {
		ges = vmalloc(sizeof(u8) * P5_X_GESTURE_INFO_LENGTH);
		ipio_memcpy(ges, buf, len, P5_X_GESTURE_INFO_LENGTH);
	} else {
		ges = vmalloc(sizeof(u8) * P5_X_GESTURE_INFO_LENGTH_HIGH_RESOLUTION);
