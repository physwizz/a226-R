...
int ili_report_handler(void)
{
	...
	/**************************************************
		add for gesture wakeup in gesture debug mode by ming
	**************************************************/
	...
	**************************************************/
	...
	case P5_X_DEMO_AXIS_PACKET_ID:
		ili_report_ap_mode(&trdata[P5_X_DEMO_MODE_PACKET_INFO_LEN], (rlen-P5_X_DEMO_MODE_PACKET_INFO_LEN - P5_X_DEMO_MODE_AXIS_LEN- P5_X_DEMO_MODE_STATE_INFO ));
		break;
...
static void ili_update_tp_module_info(void)
{
	...
	if (module == 0 || ilits->md_fw_ili_size < ILI_FILE_HEADER) {
		...
		if (strstr(saved_command_line,"androidboot.board_id=S96806EA1")) { //for Korea version
			ilits->md_fw_ili = CTPM_FW_DEF_EA1;
			ilits->md_fw_ili_size = sizeof(CTPM_FW_DEF_EA1);
		} else {
