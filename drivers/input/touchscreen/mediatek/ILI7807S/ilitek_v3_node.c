...
int wt_tp_openshort(void)
{
	int ret = 0;
	bool esd_en = ilits->wq_esd_ctrl,bat_en = ilits->wq_bat_ctrl;

	ILI_INFO("Run MP test\n");

	mutex_lock(&ilits->touch_mutex);

	/* Create the directory for mp_test result */
	if ((dev_mkdir(CSV_LCM_ON_PATH, S_IRUGO | S_IWUGO | S_IXUGO)) != 0)
		ILI_ERR("Failed to create directory for mp_test\n");
	...
	if(ret < 0){
		ILI_INFO("MP test fail\n");
		ret = -1;
	}else{
		ILI_INFO("MP test pass\n");
		ret = 0;
	}
	return ret;
}
...
static long ilitek_node_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_COMPAT_IOCTL_REPORT_RATE_SET:
		...
		return ret;
	case ILITEK_COMPAT_IOCTL_REPORT_RATE_GET:
		ILI_DBG("compat_ioctl: convert report rate get\n");
		ret = filp->f_op->unlocked_ioctl(filp, ILITEK_IOCTL_REPORT_RATE_GET, (unsigned long)compat_ptr(arg));
		return ret;
	default:
